...
void get_city_info( struct vitalstat *pvs) 
{
	...
	while (!answered) //while'dan ç?kmak için answered = 1 olmal?
	if (!pvs->UScitizen) //eger UScitizen=0 ise
	{ 
...
answered =                                                  double_check( pvs->location.nationality); 
	...
	{ 
		...
		answered =
double_check( pvs->location.city_of_birth ); 
	}
}
