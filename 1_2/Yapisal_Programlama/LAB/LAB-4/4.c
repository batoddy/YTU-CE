...
void printFlights(Flight *head)
{
    ...
    while (temp != NULL)
    {
        printf("%d- %d %s %s %s %d %d\n", idx, temp->flightID, temp->departureCity, temp->arrivalCity, temp->departureTime, temp->dayType, temp->runway);
        temp = temp->next;
        idx++;
    }
}
...
int main()
{
    ...
    for (int i = 1; i <= 10; i++)
    {
        ...
        while (temp != NULL)
        {
            if (temp->runway == i)
            {
                Flight *newFlight = createFlight(temp->flightID, temp->departureCity, temp->arrivalCity, temp->departureTime, temp->dayType, temp->runway);
                insertFlight(&runwayFlights, newFlight);
            }
            temp = temp->next;
        }
