...
void AlertSystem::sendAlert(AlertType type,
    ...
    sendAlert(type, src, message, value);
...
void AlertSystem::dispatchAlert(const AlertInfo& alert) {
    switch (alert.type) {
        case ALERT_DUSME:
            handleDusmeAlert(alert);
            break;

        case ALERT_NABIZ_DUSUK:
            handleNabizDusukAlert(alert);
            break;

        case ALERT_NABIZ_YUKSEK:
            handleNabizYuksekAlert(alert);
            break;

        case ALERT_HAREKETSIZLIK:
            handleHareketsizlikAlert(alert);
            break;

        case ALERT_MANUEL:
            handleManuelAlert(alert);
            break;

        default:
            handleSistemAlert(alert);
            break;
    }
}

void AlertSystem::handleDusmeAlert(const AlertInfo& alert) {
    Serial.println(">>> DUSME HANDLER CALISTI <<<");
    // Örn:
    // buzzer.start();
    // gsm.sendSOS(alert);
}

void AlertSystem::handleNabizDusukAlert(const AlertInfo& alert) {
    Serial.println(">>> NABIZ DUSUK HANDLER <<<");
}

void AlertSystem::handleNabizYuksekAlert(const AlertInfo& alert) {
    Serial.println(">>> NABIZ YUKSEK HANDLER <<<");
}

void AlertSystem::handleHareketsizlikAlert(const AlertInfo& alert) {
    Serial.println(">>> HAREKETSIZLIK HANDLER <<<");
}

void AlertSystem::handleManuelAlert(const AlertInfo& alert) {
    Serial.println(">>> MANUEL ACIL DURUM HANDLER <<<");
}

void AlertSystem::handleSistemAlert(const AlertInfo& alert) {
    Serial.println(">>> SISTEM ALERT HANDLER <<<");
}
